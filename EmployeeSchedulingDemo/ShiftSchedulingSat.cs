...
public class ShiftSchedulingSat
{
    ...
    public static void SolveShiftScheduling1()
    {
        int numEmployees = 8;
        int numWeeks = 3;
                                  ...
                                  int hardMax, int maxCost, string prefix)
    {
        ...
        foreach (var length in Range(1, hardMin))
        {
            foreach (var start in Range(works.Length - length + 1))
            {
                ...
                //từ start đến lenght phải có 1 giá trị true
                // 1: 0(start), 000000, 0(end) -> skip (làm liên tục) (important)
                //2: 0,..1...,0 true có nghỉ ít nhất 1 ngày
                //3: 0,11111,0 nghỉ liên tục
                //4: 1,xxxxx,0
                //5: 1,xxxxx,1
                //6: 0,xxx,1 
                // k phủ 2 đầu
        ...
        foreach (var start in Range(works.Length - hardMax))
        {
            ...
            // 111111111111111 -> 0000000000 -> false
            //001010111
